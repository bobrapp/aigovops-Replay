...
function getExpressRoutesFromRuntime(): Set<string> {
  ...
  // Always rebuild before introspecting so the route table reflects the
  // current TypeScript source, not a stale dist/ artifact.  This is what
  // makes the drift checker source-of-truth-safe: if a developer edits a
  // route file but forgets to rebuild, we still see the new registration.
  // Skip only when SKIP_API_BUILD=1 is set (used by tests that already
  // know the bundle is fresh).
  if (process.env.SKIP_API_BUILD !== "1") {
    console.log("[spec-drift] Building api-server (fresh route table)…");
    const build = spawnSync(
      "pnpm",
      ["--filter", "@workspace/api-server", "run", "build"],
      { encoding: "utf8", stdio: "inherit", cwd: ROOT },
    );
    if (build.status !== 0) {
      console.error(
        `[spec-drift] api-server build failed (exit ${build.status}); cannot introspect routes.`,
      );
      process.exit(1);
    }
  }
  ...
  if (!existsSync(apiBin)) {
    console.error(
      `[spec-drift] api-server bundle still not found at ${apiBin} after build.`,
    );
    process.exit(1);
  }
...
// ── Stage 3b: response body shape validation (all methods) ──────────────────
// Stage 3 only catches component schemas whose name is ALSO a Zod export name
// (true for request-body schemas like CreateInteractionBody, but rarely for
// response schemas — orval renames them, e.g. InteractionList →
// ListInteractionsResponse).  Stage 3b closes the gap by resolving the
// expected `{Prefix}Response` Zod export for *every* operation (not just GET)
// whose 2xx response references a named component schema, and field-comparing
// it against the underlying component.
//
// Method-aware behaviour:
//   • GET — Orval always emits {Prefix}Response.  If the Zod export is
//     missing, that's a Stage 2 failure (already reported); we silently skip
//     the shape comparison here to avoid double-reporting.
//   • Non-GET (POST/PUT/PATCH/DELETE) — Orval *deduplicates*: if the response
//     references the same component schema as a previously-seen operation,
//     the second response export is omitted and the type is inlined.  In
//     that case the absence of `{Prefix}Response` is expected and not an
//     error — we silently skip.  When the export DOES exist (e.g. the
//     component is unique to this operation), we run the full field check
//     so non-GET response drift IS caught when orval owns the export.
...
for (const [_path, methods] of Object.entries(spec.paths ?? {})) {
  for (const [method, operation] of Object.entries(methods)) {
    ...
    for (const [code, resp] of Object.entries(op.responses ?? {})) {
      ...
      // Missing export is OK for non-GET (orval dedupe) and is a Stage 2
      // failure for GET that's already been reported above — either way,
      // skip silently rather than emitting a misleading "missing field"
      // error that would be a duplicate.
      ...
      for (const prop of allProps) {
        if (!zodFields.has(prop)) {
          responseShapeViolations.push(
            `${method.toUpperCase()} ${_path} :: ${componentName} (→ ${zodExportName}).${prop}: ` +
      ...
      for (const field of required) {
        ...
        if (zodFields.get(field)!.optional) {
          responseShapeViolations.push(
            `${method.toUpperCase()} ${_path} :: ${componentName} (→ ${zodExportName}).${field}: ` +
...
if (!hasDrift) {
  ...
  console.log(
    `    Shape check  (Stage 3): ${schemasChecked} request body schema(s) + ${responseShapesChecked} response schema(s) (all methods) field-compared (required/optional status, property presence).`,
  );
  process.exit(0);
} else {
  const total =
